  ...
  <div>
    <v-dialog v-model="createDialog" max-width="400" persistent>
        <v-card class="pt-3 pb-3">
            ...
            <v-card-text>
                <v-text-field label="Title" placeholder="Describe it in a few words." v-model="request_title"></v-text-field>
                <v-text-field multi-line label="Detail" placeholder="And expound it more here." v-model="detail"></v-text-field>
            </v-card-text>
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="indigo darken-1" flat="flat" @click="send" :loading="isLoading">Post</v-btn>
                <v-btn color="red darken-1" flat="flat" @click="$emit('update:createDialog', false)">Cancel</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
